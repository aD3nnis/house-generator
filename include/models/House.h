...
public:
  void set_bedroom_count(int bed_count) { bedroom.resize(bed_count); }
  void set_bathroom_count(int bath_count) { bathroom.resize(bath_count); }
  // percent of space they take up
  void set_room_percentages();

  float get_living_percent() { return livingPercent; }
  float get_total_bedroom_percent() { return totalBedroomPercent; }
  float get_total_bathroom_percent() { return totalBathroomPercent; }
  float get_kitchen_percent() { return kitchenPercent; }
  ...
  void create_rooms(House &house1, DimensionGenerator &dimension_gen, int totalSqft);
};
#endif