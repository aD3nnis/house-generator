...
void House::create_rooms(House &house1, DimensionGenerator &dimension_gen,
                         int totalSqft) {
                            
  house1.get_livingroom().set_room_sqft(house1.get_living_percent(), totalSqft);
  house1.get_livingroom().update_dimensions(dimension_gen);
  ...
  house1.get_kitchen().update_dimensions(dimension_gen);
  ...
  for (int i = 0; i < house1.get_bedroom_count(); i++) {
    house1.get_bedroom(i).set_room_sqft(house1.get_bedroom_percents()[i],
                                        totalSqft);
    house1.get_bedroom(i).update_dimensions(dimension_gen);
  ...
  for (int i = 0; i < house1.get_bathroom_count(); i++) {
    house1.get_bathroom(i).set_room_sqft(house1.get_bathroom_percents()[i],
                                         totalSqft);
    house1.get_bathroom(i).update_dimensions(dimension_gen);
